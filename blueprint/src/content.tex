...
\lean{unique_topology_of_finiteDimensional}
	The usual topology on $\mathbb{R}^{n}$ is the only Hausdorff topology that makes it a topological
  vector space.
\end{theorem}
...
In this chapter, all Banach spaces are supposed to be \emph{non-zero}.
...
\begin{definition}\label{def:bolognese} A topological vector space is Bolognese if the convex open
...
\begin{theorem}\label{thm:lp_dual_zero} For every $0<p<1$ the dual space $\bigl(\ell^{p}\bigr)^\ast$ is equal to $0$.
\end{theorem}
...
\begin{corollary}\label{cor:L_p_not_bolognese} The space $L^{p}([0,1])$ of equivalence classes of measurable functions $f\colon [0,1]\to\mathbb{R}$ satisfying $\int_0^1\lvert f(x)\rvert ^{p}\mathrm{d}x < \infty$ \textup{(}with respect to the Lebesgue measure\textup{)} is not Bolognese.
\end{corollary}
