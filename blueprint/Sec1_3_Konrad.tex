...
\footnote{See Theorem 2.7 in \href{https://kconrad.math.uconn.edu/blurbs/topology/finite-dim-TVS.pdf}{https://kconrad.math.uconn.edu/blurbs/topology/finite-dim-TVS.pdf}.}
\end{Example}
...
\begin{Theorem}\label{thm:the28} Let $V$ be a Banach space. For each non-zero $v \in V$, there is a $\varphi \in V^{*}$ such that $\varphi(v) \neq 0$. Thus, given distinct $v$ and $w$ in $V$, there is a $\varphi \in V^{*}$ such that $\varphi(v) \neq \varphi(w)$.
\end{Theorem}
Theorem~\ref{thm:the28} is a special case of the Hahn--Banach theorem, and can be found in texts on functional analysis. Even this special case can't be proved in a constructive way (when $V$ is infinite-dimensional). Its general proof depends on the axiom of choice.
...
\begin{Example}For $n \geq 1$, the usual topology on $\mathbf{R}^{n}$ makes it a topological vector space. Using instead the discrete topology, indicated by writing $\mathbf{R}_{d}^{n}$, does not make a topological vector space: for nonzero $\mathbf{v}_{0}$ in $\mathbf{R}_{d}^{n},\left\{\mathbf{v}_{0}\right\}$ is open and scalar multiplication $\mathbf{R} \times\left\{\mathbf{v}_{0}\right\} \rightarrow \mathbf{R}^{n}$ is continuous, so the inverse image of $\left\{\mathbf{v}_{0}\right\}$ in $\mathbf{R} \times\left\{\mathbf{v}_{0}\right\}$ is open. That is $\left\{\left(1, \mathbf{v}_{0}\right)\right\}$, and $\mathbf{R} \times\left\{\mathbf{v}_{0}\right\}$ is homeomorphic to $\mathbf{R}$, so $\{1\}$ is open in $\mathbf{R}$, which is not true. In fact, the usual topology on $\mathbf{R}^{n}$ is the only Hausdorff topology that makes it a topological vector space\footnote{See Theorem 2.7 in \href{https://kconrad.math.uconn.edu/blurbs/topology/finite-dim-TVS.pdf}{https://kconrad.math.uconn.edu/blurbs/topology/finite-dim-TVS.pdf}.}
		 and the only topological vector space over $\mathbf{R}$ with the discrete topology is the zero space\footnote{${ }^{3}$ See \href{https://math.stackexchange.com/questions/492483}{https://math.stackexchange.com/questions/492483}.
}.
\end{Example}
...
\end{Definition}
Theorem~\ref{thm:the28} generalizes to all locally convex spaces, as follows.
...
\end{Theorem}
\begin{proof} See the chapters on locally convex spaces in~\cite{Con90} or~\cite{ReeBar80}.
\end{proof}
Let's meet some topological vector spaces that are not locally convex.
\begin{Example} Let $L^{1 / 2}[0,1]$ be the set of all measurable functions $f\colon [0,1] \rightarrow \mathbf{R}$ such that $\int_{0}^{1}|f(x)|^{1 / 2} \mathrm{~d} x<\infty$, with functions equal almost everywhere identified. (We need to make such an identification, since integration does not distinguish between two functions that differ on a set of measure 0 .)
...
This method of constructing topological vector spaces that are not locally convex is due to Tychonoff~\cite[pp.~768--769]{Tyc35}. He used the space $\ell^{1 / 2}=\left\{\left(x_{i}\right): \sum_{i \geq 1} \sqrt{x_{i}}<\infty\right\}$ rather than the function space $L^{1 / 2}[0,1]$.
...
\begin{Theorem}\label{thm:dual_Lp_zero} For $0<p<1, L^{p}[0,1]^{*}=\{0\}$. That is, the only continuous linear map $L^{p}[0,1] \rightarrow \mathbf{R}$ is 0 .
\end{Theorem}
...
\section{Other Spaces with Trivial Dual}
We recall the definition of $L^{p}$-spaces for a measure space and then extend Theorem~\ref{thm:dual_Lp_zero} quite generally.

\begin{Definition} Let $(X, \mathcal{M}, \mu)$ be a measure space. For $0<p$, set
\[
L^{p}(\mu) =\left\{f\colon X \rightarrow \mathbb{R} \text{ such that }f\text{ is measurable and } \int_{X}|f|^{p} \mathrm{~d} \mu<\infty\right\}\big\slash\sim
\]
where functions that are equal almost everywhere are identified with one another.
\end{Definition}

The spaces $L^{p}(\mu)$ are actually metric spaces, with metric
\[
d(f, g)= \begin{cases}
...
\bibliography{biblio_Sec1_3_Konrad}
