...
lemma mono {f : α → β} {F G : Filter α} (h : F ≤ G) : F.map f ≤ G.map f := /- fun _ H ↦ h H -/by
  simp [Filter.le_def] at h ⊢ --remove after first trial
  intro U hU
...
/- During the lecture, write `Tendsto (g ∘ f) G H` to show Lean is clever! -/
lemma Tendsto_comp {f : α → β} {g : β → γ} {F : Filter α} {G : Filter β} {H : Filter γ}
