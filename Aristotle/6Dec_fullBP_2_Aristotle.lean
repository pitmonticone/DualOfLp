...
theorem unique_topology_Rn (n : ℕ) (t : TopologicalSpace (Fin n → ℝ))
  ...
  -- Since the identity map is continuous from t to the product topology, t is finer than the
  -- product topology.
...
theorem banach_exists_mem_dual_eval_ne_zero {V : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
    [CompleteSpace V] (v : V) (hv : v ≠ 0) :
    ∃ φ : V →L[ℝ] ℝ, φ v ≠ 0 := by
  have := exists_dual_vector ℝ v hv;
  exact ⟨ this.choose, this.choose_spec.2.symm ▸ by simpa using hv ⟩

/-
Let $V$ be a Banach space and let $v\neq w$ be two distinct elements of $V$. There is a $\varphi
\in V^*$ such that $\varphi(v) \neq \varphi(w)$.
-/
theorem banach_exists_mem_dual_eval_ne {V : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
    [CompleteSpace V] (v w : V) (h : v ≠ w) : ∃ φ : V →L[ℝ] ℝ, φ v ≠ φ w := by
  -- Since $v \neq w$, we have $v - w \neq 0$.
  have h_ne : v - w ≠ 0 := by
...
theorem bolognese_iff_lc {V : Type*} [AddCommGroup V] [Module ℝ V] [TopologicalSpace V]
    [ContinuousSMul ℝ V] [ContinuousAdd V] :
    Bolognese V ↔ LocallyConvexSpace ℝ V := by
  constructor <;> intro h <;> rw [ Bolognese ] at *;
  · -- To prove local convexity, we need to show that every point has a neighborhood base
    -- consisting of convex sets.
    have h_local_convex : ∀ x : V, ∀ U : Set V, IsOpen U → x ∈ U → ∃ C : Set V, IsOpen C ∧
        Convex ℝ C ∧ x ∈ C ∧ C ⊆ U := by
  ...
  · intro x U hx hU
    obtain ⟨K, hK_convex, hK_neighborhood, hK_subset⟩ : ∃ K : Set V, Convex ℝ K ∧ IsOpen K ∧
        x ∈ K ∧ K ⊆ U := by
      have := h.1 x;
...
theorem Bolognese_of_Banach {V : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V] [CompleteSpace V] :
  ...
  have h_locally_convex : LocallyConvexSpace ℝ V := by
    infer_instance;
  -- Since $V$ is locally convex, we can apply the lemma `bolognese_iff_lc` to conclude that it is
  -- Bolognese.
...
-/
theorem bolognese_exists_mem_dual_eval_ne {V : Type*} [AddCommGroup V] [Module ℝ V]
    [TopologicalSpace V]
...
/-
Let $(X,\mu)$ be a measure space. For every 0 < p < 1 the space $L^p(\mu)$ is a metric space with metric
-/
open MeasureTheory ENNReal
...
def MeasureTheory.Measure.FiniteRange {α : Type*} [MeasurableSpace α] (μ : Measure α) : Prop := (Set.range (fun s => μ s)).Finite

variable {α E : Type*} [MeasurableSpace α] [NormedAddCommGroup E] [NormedSpace ℝ E] {μ : Measure α} {p : ℝ≥0∞}

#check MeasureTheory.Lp.instModule
#synth Module ℝ (Lp E p μ)

/-
If a measure assumes finitely many values and is not zero, then it has an atom.
-/
open MeasureTheory ENNReal Set

variable {α : Type*} [MeasurableSpace α] {μ : Measure α}

def MeasureTheory.Measure.IsAtom (μ : Measure α) (s : Set α) : Prop :=
  MeasurableSet s ∧ μ s ≠ 0 ∧ ∀ t, t ⊆ s → MeasurableSet t → μ t = 0 ∨ μ t = μ s

lemma exists_atom_of_finite_range (h : MeasureTheory.Measure.FiniteRange μ) (hμ : μ ≠ 0) :
    ∃ s, MeasureTheory.Measure.IsAtom μ s := by
      -- Consider the set of all measurable subsets of $X$ with positive measure.
      let S := {s : Set α | MeasurableSet s ∧ 0 < μ s};
      -- Since $\mu$ is not zero, there exists some $s \in S$.
      obtain ⟨s, hs⟩ : ∃ s ∈ S, ∀ t ∈ S, μ t ≥ μ s := by
        have hS_nonempty : S.Nonempty := by
          -- Since μ is not zero, there exists some set s with μ s > 0.
          obtain ⟨s, hs⟩ : ∃ s : Set α, MeasurableSet s ∧ 0 < μ s := by
            contrapose! hμ;
            ext s hs; specialize hμ s hs; aesop;
          exact ⟨ s, hs ⟩;
        -- Since the range of μ is finite, the set of measures of sets in S is also finite.
        have hT_finite : Set.Finite (Set.image (fun s => μ s) S) := by
          exact h.subset ( Set.image_subset_range _ _ );
        have := hT_finite.toFinset.exists_min_image id; aesop;
      refine' ⟨ s, _ ⟩;
      constructor <;> aesop;
      -- Since $t \subseteq s$, we have $\mu t \leq \mu s$. Also, by the minimality of $\mu s$, if
      -- $\mu t > 0$, then $\mu s \leq \mu t$.
      by_cases h_pos : 0 < μ t;
      · exact Or.inr ( le_antisymm ( MeasureTheory.measure_mono a ) ( right t a_1 h_pos ) );
      · aesop

/-
If a measure is atomless and assumes finitely many values, then it is the zero measure.
-/
open MeasureTheory ENNReal Set

variable {α : Type*} [MeasurableSpace α] {μ : Measure α}

def MeasureTheory.Measure.Atomless (μ : Measure α) : Prop := ∀ s, ¬ MeasureTheory.Measure.IsAtom μ s

lemma eq_zero_of_atomless_finite_range (h : MeasureTheory.Measure.FiniteRange μ)
    (h_atomless : MeasureTheory.Measure.Atomless μ) : μ = 0 := by
      -- Assume that μ is not the zero measure. Then there exists a set s with μ(s) > 0.
      by_contra h_nonzero
      obtain ⟨s, hs⟩ : ∃ s, μ s > 0 := by
        exact not_forall_not.mp fun h => h_nonzero <| MeasureTheory.Measure.ext fun s => by aesop;
      have h_contra : ∃ s, MeasureTheory.Measure.IsAtom μ s := by
        exact?;
      exact h_contra.elim fun s hs => h_atomless s hs

/-
If a measure assumes finitely many values, then there cannot be an infinite sequence of disjoint
  atoms of finite measure.
-/
open MeasureTheory ENNReal Set

variable {α : Type*} [MeasurableSpace α] {μ : Measure α}

lemma not_infinite_disjoint_atoms_finite_measure (h : MeasureTheory.Measure.FiniteRange μ) :
    ¬ ∃ f : ℕ → Set α, (∀ n, MeasureTheory.Measure.IsAtom μ (f n)) ∧ (∀ n, μ (f n) ≠ ∞) ∧
        Pairwise (fun n m => Disjoint (f n) (f m)) := by
      -- Let's obtain the minimum positive measure value ε from the finite range of μ.
      obtain ⟨ε, hε_pos, hε_min⟩ : ∃ ε > 0, ∀ s, μ s ≠ 0 → μ s ≥ ε := by
        have h_nonzero_finite : Set.Finite {x ∈ Set.range μ | x ≠ 0} := by
          exact h.subset fun x hx => hx.1;
        by_cases h_nonzero_empty : {x ∈ Set.range μ | x ≠ 0} = ∅;
        · exact ⟨ 1, zero_lt_one, fun s hs => False.elim <| h_nonzero_empty.subset ⟨ ⟨ s, rfl ⟩, hs ⟩ ⟩;
        · have := Finset.exists_min_image ( h_nonzero_finite.toFinset ) id
            ( Finset.nonempty_of_ne_empty ( by simpa ) ) ; aesop;
          exact ⟨ μ w_1, pos_iff_ne_zero.mpr right_1, right ⟩;
      -- Let's obtain the maximum finite measure value M from the finite range of μ.
      obtain ⟨M, hM_max⟩ : ∃ M ≠ ⊤, ∀ s, μ s ≠ ⊤ → μ s ≤ M := by
        have h_finite_range : Set.Finite {x | x ≠ ⊤ ∧ ∃ s, μ s = x} := by
          exact h.subset fun x hx => by aesop;
        by_cases h_nonempty : {x | x ≠ ⊤ ∧ ∃ s, μ s = x}.Nonempty;
        · exact ⟨ h_finite_range.toFinset.max' ⟨ h_nonempty.some, h_finite_range.mem_toFinset.mpr
            h_nonempty.choose_spec ⟩, h_finite_range.mem_toFinset.mp
              ( h_finite_range.toFinset.max'_mem _ ) |>.1, fun s hs => Finset.le_max' _ _
                ( h_finite_range.mem_toFinset.mpr ⟨ hs, s, rfl ⟩ ) ⟩;
        · simp_all +decide [ Set.Nonempty ];
          exact ⟨ 0, by simp +decide, fun s hs => False.elim <| h_nonempty ( μ s ) hs s rfl ⟩;
      -- Assume for contradiction that there exists an infinite sequence of disjoint atoms of finite
      -- measure.
      by_contra h_contra
      obtain ⟨f, hf_atom, hf_finite, hf_disjoint⟩ := h_contra

      -- Let's consider the measure of the union of these atoms.
      have h_union_measure : ∀ n, μ (⋃ i ≤ n, f i) ≥ (n + 1) * ε := by
        intro n
        have h_union_measure_step : μ (⋃ i ≤ n, f i) = ∑ i ∈ Finset.range (n + 1), μ (f i) := by
          rw [ show ( ⋃ i : ℕ, ⋃ ( hi : i ≤ n ), f i ) =
            ⋃ i ∈ Finset.range ( n + 1 ), f i by ext; simp +decide [ Nat.lt_succ_iff ] ] ;
            rw [ MeasureTheory.measure_biUnion_finset ] ; aesop;
          · exact fun i hi j hj hij => hf_disjoint hij;
          · exact fun i _ => ( hf_atom i ).1;
        aesop;
        exact le_trans ( by simp +decide ) ( Finset.sum_le_sum fun i _ => hε_min _
          ( hf_atom i |>.2.1 ) );
      -- Since each $f(i)$ has finite measure, $S_k$ has finite measure.
      have h_union_finite : ∀ n, μ (⋃ i ≤ n, f i) ≠ ⊤ := by
        intro n hn_inf; specialize h_union_measure n; simp_all +decide [ ENNReal.mul_eq_top ] ;
        -- Since the measure of the union of the atoms is infinite, but each atom has finite measure,
        -- this leads to a contradiction.
        have h_contradiction : μ (⋃ i ≤ n, f i) ≤ ∑ i ∈ Finset.range (n + 1), μ (f i) := by
          exact le_trans ( MeasureTheory.measure_mono ( show ⋃ i, ⋃ ( _ : i ≤ n ), f i ⊆
            ⋃ i ∈ Finset.range ( n + 1 ), f i from fun x hx =>
            by rcases Set.mem_iUnion₂.1 hx with ⟨ i, hi, hx ⟩ ; exact Set.mem_iUnion₂.2 ⟨ i, Finset.mem_range_succ_iff.2 hi, hx ⟩ ) ) ( MeasureTheory.measure_biUnion_finset_le _ _ ) |> le_trans <| by simp +decide [ Finset.sum_range_succ ] ;
        exact hn_inf.not_lt ( lt_of_le_of_lt h_contradiction ( ENNReal.sum_lt_top.mpr fun i hi => lt_top_iff_ne_top.mpr ( hf_finite i ) ) );
      -- Choose $n$ such that $(n + 1) * ε > M$.
      obtain ⟨n, hn⟩ : ∃ n : ℕ, (n + 1) * ε > M := by
        cases' ENNReal.lt_iff_exists_nnreal_btwn.mp hε_pos with ε' hε';
        cases' ENNReal.lt_iff_exists_nnreal_btwn.mp ( show M < ⊤ from lt_top_iff_ne_top.mpr hM_max.1 ) with M' hM' ; aesop;
        cases' exists_nat_gt ( M' / ε' ) with n hn ; use n ; rw [ ← NNReal.coe_lt_coe ] at * ; aesop;
        rw [ div_lt_iff₀ ] at hn <;> norm_cast at * ; aesop;
        exact lt_of_lt_of_le hM' ( le_trans ( mod_cast hn.le )
          ( mul_le_mul_of_nonneg_left right_1.le <| by positivity ) ) |>
          lt_of_lt_of_le <| mul_le_mul_of_nonneg_right ( le_add_of_nonneg_right zero_le_one ) <|
          by positivity;
      exact hn.not_le ( le_trans ( h_union_measure n ) ( hM_max.2 _ ( h_union_finite n ) ) )


/-
Addition is continuous in L^p for 0 < p < 1.
-/
open MeasureTheory ENNReal
...
/-- Addition is continuous in L^p for 0 < p < 1. -/
instance Lp.instContinuousAdd_p_lt_one [Fact (0 < p)] [Fact (p < 1)] :
    ContinuousAdd (Lp E p μ) := by
      -- To prove continuity of addition, we can use the fact that the addition of two functions in L^p is continuous.
      have h_add_cont : Continuous (fun (fg : Lp E p μ × Lp E p μ) => fg.1 + fg.2) := by
        refine' continuous_iff_continuousAt.mpr _;
        bound;
        -- Apply the triangle inequality to the L^p norm of (f + g) - (f' + g').
        have h_triangle : ∀ f g f' g' : Lp E p μ, (eLpNorm (f + g - (f' + g')) p μ).toReal ^ p.toReal ≤ (eLpNorm (f - f') p μ).toReal ^ p.toReal + (eLpNorm (g - g') p μ).toReal ^ p.toReal := by
          simp +zetaDelta at *;
          -- Apply the subadditivity of the L^p norm for 0 < p < 1.
          have h_subadd : ∀ f g : Lp E p μ, (eLpNorm (f + g) p μ).toReal ^ p.toReal ≤ (eLpNorm f p μ).toReal ^ p.toReal + (eLpNorm g p μ).toReal ^ p.toReal := by
            intro f g;
            have := @Lp.dist_triangle_p_lt_one α E _ _ μ p;
            simpa using this f 0 ( -g );
          -- Apply the subadditivity of the L^p norm to the functions (a - a_2) and (a_1 - a_3).
          intros a ha a_1 ha_1 a_2 ha_2 a_3 ha_3
          have h_diff : (eLpNorm (↑(a + a_1 - (a_2 + a_3))) p μ).toReal ^ p.toReal = (eLpNorm (↑((a - a_2) + (a_1 - a_3))) p μ).toReal ^ p.toReal := by
            simp +decide only [sub_add_sub_comm];
          convert h_subadd ⟨ a - a_2, ?_ ⟩ ⟨ a_1 - a_3, ?_ ⟩ using 1 <;> aesop;
        rw [ Metric.continuousAt_iff ];
        intro ε ε_pos;
        use ε / 2;
        simp +decide [ Prod.dist_eq, dist_eq_norm ];
        refine' ⟨ ε_pos, fun f hf g hg hf' hg' => _ ⟩;
        -- Apply the triangle inequality to the L^p norm of (f + g) - (val + val_1).
        have h_triangle : (eLpNorm (f + g - (val + val_1)) p μ).toReal ^ p.toReal ≤ (eLpNorm (f - val) p μ).toReal ^ p.toReal + (eLpNorm (g - val_1) p μ).toReal ^ p.toReal := by
          convert h_triangle ⟨ f, hf ⟩ ⟨ g, hg ⟩ ⟨ val, property ⟩ ⟨ val_1, property_1 ⟩ using 1;
        exact lt_of_le_of_lt h_triangle ( by linarith! );
      -- Apply the continuity of the addition operation in the product space to the L^p space.
      apply ContinuousAdd.mk;
      convert h_add_cont using 1

/-
Negation is continuous in L^p for 0 < p < 1.
-/
open MeasureTheory ENNReal
...
/-- Negation is continuous in L^p for 0 < p < 1. -/
instance Lp.instContinuousNeg_p_lt_one [Fact (0 < p)] [Fact (p < 1)] :
    ContinuousNeg (Lp E p μ) := by
      -- Since the negation is continuous in the metric space L^p, it is also continuous in the TVS structure.
      have h_neg_cont : Continuous (fun f : Lp E p μ => -f) := by
        -- The metric is symmetric, so the negation function is continuous.
        have h_symm : ∀ f g : Lp E p μ, (eLpNorm (f - g) p μ).toReal ^ p.toReal = (eLpNorm (g - f) p μ).toReal ^ p.toReal := by
          -- Since the absolute value is symmetric, we have |f - g| = |g - f|.
          have h_abs_symm : ∀ f g : Lp E p μ, (eLpNorm (f - g) p μ).toReal = (eLpNorm (g - f) p μ).toReal := by
            intro f g;
            -- Since the norm of a vector is the same as the norm of its negative, we have |f - g| = |g - f|.
            have h_abs : ∀ x : α →ₘ[μ] E, (eLpNorm (↑↑x) p μ).toReal = (eLpNorm (↑↑(-x)) p μ).toReal := by
              bound;
              rw [ MeasureTheory.eLpNorm_congr_ae ( MeasureTheory.AEEqFun.coeFn_neg _ ) ] ; simp +decide [ Real.norm_eq_abs ];
            convert h_abs ( f - g ) using 1 ; norm_num;
          exact fun f g => congr_arg ( · ^ p.toReal ) ( h_abs_symm f g );
        -- Since the metric is symmetric, the distance between -f and -g is the same as the distance between f and g. Therefore, if we can find a δ such that the distance between f and g is less than δ implies the distance between -f and -g is less than ε, then the negation function is continuous.
        have h_neg_cont : ∀ ε > 0, ∃ δ > 0, ∀ f g : Lp E p μ, (eLpNorm (f - g) p μ).toReal ^ p.toReal < δ → (eLpNorm (-f - (-g)) p μ).toReal ^ p.toReal < ε := by
          intro ε hε;
          norm_num +zetaDelta at *;
          exact ⟨ ε, hε, fun f hf g hg hfg => by simpa [ neg_add_eq_sub ] using h_symm f hf g hg ▸ hfg ⟩;
        -- Apply the hypothesis h_neg_cont to conclude the continuity of the negation function.
        apply Metric.continuous_iff.mpr;
        intro f ε hε;
        obtain ⟨ δ, hδ, hδ' ⟩ := h_neg_cont ε hε;
        exact ⟨ δ, hδ, fun g hg => by simpa only [ dist_eq_norm ] using hδ' g f hg ⟩;
      exact?